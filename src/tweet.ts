...
const s3Client = () => {
  ...
  if (!_s3Client)
    _s3Client = new Minio.Client({
      endPoint:
        process.env.AWS_DEFAULT_REGION + '.' + process.env.AWS_S3_ENDPOINT,
...
const downloadCsv = <T>(url): Promise<T[]> =>
  new Promise((resolve) => {
    https.get(url, (res) => {
      const result: T[] = []
      res
        .pipe(csv.parse({ columns: true }))
        .on('data', (data) => result.push(data as T))
        .on('end', () => resolve(result))
    })
  })

const tweet = async (iso3c, jurisdiction, max) => {
  if (iso3c.startsWith('USA-') || iso3c.startsWith('DEU-')) return
  console.log(`Tweeting for ${jurisdiction}...`)
  await downloadImage(`${url_base}${iso3c}.png`, './out/twitter.png')
  const message =
    `📊 Mortality data for ${jurisdiction} just updated!\n` +
...
type Country = { iso3c: string; max_date: string }

const main = async () => {
  fs.mkdirSync('./out', { recursive: true })
  const countries: CountryFull[] = await downloadCsv<CountryFull>(
    'https://s3.mortality.watch/data/mortality/world_meta.csv'
  )
  const max_dates_old: Country[] = await downloadCsv<Country>(
    'https://s3.mortality.watch/data/mortality/world_max_date.csv'
  )
  const unique_countries: Country[] = []
  ...
  for (const country of unique_countries) {
    ...
    const is_state =
      country.iso3c.includes('DEU-') || country.iso3c.includes('USA-')
    if (!is_state && (!old || dn)) updated.push(country)
  }
